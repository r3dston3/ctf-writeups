...
We have pcap file, first off all I sorted packets with http protocol.

After that i get http follow an get this request :
...
so, after analys i reversed the string and decode it with base64. After that i got this script : 
...
So first of all let's decode $part1 and get **HTB{fr33_N17r0G3n_3xp053d!_**.

I noticed that script send encrypted data with post request, after that i found information about encryption in this code and get AES key, in **base64 **= Y1dwaHJOVGs5d2dXWjkzdDE5amF5cW5sYUR1SWVGS2k=, **plain** = cWphrNTk9wgWZ93t19jayqnlaDuIeFKi.

Get back to wireshark and analys http follow of post request
...
So, we just need decrypt this with CBC mode.
...
And finally decode email field with base64, second part = b3W4r3_0f_T00_g00d_2_b3_7ru3_0ff3r5}
